...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'sat')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/sat/', args.save_name)
    ...
    ds = load_dataset(DATA_PATH, "default")['test']
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        images = [bytes_to_image(image) for image in sample['image_bytes']]
        question = '<image>\n'*len(images) + sample['question'] + '\nOptions:\n(A) {}\n(B) {}'.format(sample['answers'][0], sample['answers'][1]) + "\nAnswer with the option letter with parenthesis directly."
    ...
    if rank == 0:
        ...
        for input_example, model_output in zip(data, all_outputs):
            answer = ['A', 'B'][input_example['answers'].index(input_example['correct_answer'])]
