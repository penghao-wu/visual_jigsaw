...
// Adds: Upload Image (file input #vj-upload + drag-drop onto Image panel)
...
(function(){
  ...
  function refs(){
    return {
      ...
      tabs:   $('#vj-tabs'),
      panels: $('#vj-panels'),
      imagePanel: $('.vj-panel[data-panel="image"]'),
  ...
  function loadImageFromFile(file){
    ...
    if (S.imageObjectURL) { URL.revokeObjectURL(S.imageObjectURL); S.imageObjectURL = null; }
    const url = URL.createObjectURL(file);
    S.imageObjectURL = url;
    S.imageSrc = url;
    loadLocalImage(url);
  }

  function bindImagePanelDrop(panelEl){
    if(!panelEl) return;
  ...
  function createPaletteSlot(idx){
    ...
    makeDropZone(slot, (e, id, targetSlot)=>{
      ...
      const targetChild = targetSlot.firstChild;
  ...
  function doShuffle(){
    if (S.mode === 'view3d') return; // disabled in 3D
    if(S.mode==='image'){
  ...
  window.VJPlayground = {
    init(opts={}){
      ...
      // upload button
      if (refs().upload) {
        refs().upload.addEventListener('change', (e)=>{
          const f = e.target.files && e.target.files[0];
          if (f) loadImageFromFile(f);
          e.target.value = ''; // allow re-selecting same file
        });
      }
      // drag & drop upload on image panel
      bindImagePanelDrop(refs().imagePanel);
