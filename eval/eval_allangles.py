...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'All-Angles-Bench/data.json')
    IMAGE_ROOT = os.path.join(args.data_path, 'All-Angles-Bench')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/allangles/', args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        images = [Image.open(os.path.join(IMAGE_ROOT, image)).convert("RGB") for image in sample["image_path"]]
        question = '<image>\n'*len(images) + sample['question'] + f"\nA. {sample['A']}\nB. {sample['B']}\nC. {sample['C']}" +'\nAnswer with the option letter directly.'
