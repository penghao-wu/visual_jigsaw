...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'TUNA/TUNA-MCQ.json')
    VIDEO_ROOT = os.path.join(args.data_path, 'TUNA/video')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/tuna/', f"frame{max_frames}_pixel{pix}_"+args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        video = os.path.join(VIDEO_ROOT, sample['video']+'.mp4')
