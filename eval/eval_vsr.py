...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'vsr/test.jsonl')
    IMAGE_ROOT = os.path.join(args.data_path, 'coco')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/vsr/', args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        image_path = os.path.join(IMAGE_ROOT, 'val2017', sample['image'])
        if not os.path.exists(image_path):
            image_path = image_path.replace('val', 'train')
        image = Image.open(image_path).convert('RGB')
        question = '<image>\n' + f"Decide whether the caption correctly describes the image. Caption: {sample['caption']}" + "\nAnswer Yes or No directly."
    ...
    if rank == 0:
        ...
        for input_example, model_output in zip(data, all_outputs):
            answer = ['No', 'Yes'][input_example['label']]
            task_type = input_example['relation']
