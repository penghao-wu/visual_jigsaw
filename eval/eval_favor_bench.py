...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'FAVOR/question_perspective.json')
    VIDEO_ROOT = os.path.join(args.data_path, 'FAVOR/videos/')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/favor/', f"frame{max_frames}_pixel{pix}_"+args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        video = os.path.join(VIDEO_ROOT, sample['video_name'])
    ...
    if rank == 0:
        ...
        for input_example, model_output in zip(data, all_outputs):
            answer = ['A', 'B', 'C', 'D', 'E'][input_example['options'].index(input_example['correct_answer'])]
