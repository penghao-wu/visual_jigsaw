...
def draw_cross_pil(image, center, radius=15, color="red", thickness=3):
    """
    Draws a cross marker (circle + crosshair) on a PIL image.
    
    Args:
        image (PIL.Image): input PIL image.
        center (tuple): (x, y) coordinates of the marker.
        radius (int): size of the marker.
    ...
    # Circle
    for t in range(thickness):  # thickness by drawing multiple concentric circles
        draw.ellipse(
            (x - radius + t, y - radius + t, x + radius - t, y + radius - t),
            outline=color
        )

    # Crosshair
    draw.line((x - radius, y, x + radius, y), fill=color, width=thickness)
    draw.line((x, y - radius, x, y + radius), fill=color, width=thickness)

    return image


def vis_annotations_pil(pil_image, points, radius=30):
    img = pil_image.copy()

    p1 = (points[0][1], points[0][0])
    p2 = (points[1][1], points[1][0])

    draw_cross_pil(img, p1, radius, color="green", thickness=4)
    draw_cross_pil(img, p2, radius, color="red", thickness=4)
...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'DA-2K/all_data.json')
    IMAGE_ROOT = os.path.join(args.data_path, 'DA-2K')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/da_2k/', args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        ...
        image = vis_annotations_pil(image, sample['points'])
        question = '<image>\n' + sample['question']
    ...
    if rank == 0:
        ...
        task_type2acc = dict()
        sub_task_type2correct = defaultdict(list)
        sub_task_type2acc = dict()
        ...
        for input_example, model_output in zip(data, all_outputs):
            ...
            correct = int(model_output[0].lower() == answer.lower())
        ...
        for task_type in task_type2correct:
            ...
            print(f"\nTask: {task_type} Accuracy: {accuracy_task_type:.2f}%")
