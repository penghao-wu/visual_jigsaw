...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'OmniSpatial/all_eval_data.json')
    IMAGE_ROOT = os.path.join(args.data_path, 'OmniSpatial')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/omnispatial/', args.save_name)
    ...
    if rank == 0:
        ...
        for input_example, model_output in zip(data, all_outputs):
            answer = str(input_example['answer'])
            task_type = input_example['task_type']
            sub_task_type = input_example['sub_task_type']
            ...
            sub_task_type2correct[sub_task_type].append(correct)
            
            # Create a result dictionary for this example
            result = {
                'index': input_example['index'],
                'task_type': task_type,
        ...
        for sub_task_type in sub_task_type2correct:
            accuracy_sub_task_type = np.mean(sub_task_type2correct[sub_task_type]) * 100
            print(f"\nSub Task: {sub_task_type} Accuracy: {accuracy_sub_task_type:.2f}%")
            sub_task_type2acc[sub_task_type] = accuracy_sub_task_type
        ...
        with open(output_path, "w") as f:
            json.dump({
                ...
                'sub_task_type2acc':sub_task_type2acc,
