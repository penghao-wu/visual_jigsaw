...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'video-tt')
    VIDEO_ROOT = os.path.join(args.data_path, 'video-tt/Benchmark-AllVideos-HQ-encoded')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/videott/', f"frame{max_frames}_pixel{pix}_"+args.save_name)
    ...
    import datasets
    data = datasets.load_dataset(DATA_PATH)['test']
    data = [sample for sample in data if os.path.exists(os.path.join(VIDEO_ROOT, sample['video_id']+'.mp4'))]
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        videos = [os.path.join(VIDEO_ROOT, sample['video_id']+'.mp4')]
        question = "<video>\n"*len(videos) + sample['question']+"\nAnswer with the option letter directly."

        message = build_message_with_videos(question, videos, fps=fps, max_frames=max_frames, max_pixels=max_pixels, min_pixels=min_pixels)
    ...
    if rank == 0:
        ...
        for input_example, model_output in zip(data, all_outputs):
            ...
            task_type = input_example['capability']
