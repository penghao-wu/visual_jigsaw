...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if 'acc_reward' in batch.non_tensor_batch.keys():
        acc_reward = batch.non_tensor_batch.pop('acc_reward')
        metrics.update(
            {
            "critic/rewards/acc_reward_mean": np.mean(acc_reward),
            "critic/rewards/acc_reward_max": np.max(acc_reward),
            "critic/rewards/acc_reward_min": np.min(acc_reward),
            }
        )

    if 'acc' in batch.non_tensor_batch.keys():
        acc = batch.non_tensor_batch.pop('acc')
        metrics.update(
            {
            "critic/rewards/acc_mean": np.mean(acc),
            "critic/rewards/acc_max": np.max(acc),
            "critic/rewards/acc_min": np.min(acc),
            }
        )

    if 'format_reward' in batch.non_tensor_batch.keys():
        format_reward = batch.non_tensor_batch.pop('format_reward')
        metrics.update(
            {
            "critic/rewards/format_reward_mean": np.mean(format_reward),
            "critic/rewards/format_reward_max": np.max(format_reward),
            "critic/rewards/format_reward_min": np.min(format_reward),
            }
        )
