...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'SugarCrepe_pp/all_data.json')
    IMAGE_ROOT = os.path.join(args.data_path, 'coco/val2017')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/sugarcrepe_pp/', args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        if not os.path.exists(os.path.join(IMAGE_ROOT, sample['image'])):
            images = [Image.open(os.path.join(IMAGE_ROOT.replace('val2017', 'train2017'), sample['image']))]
        else:
            images = [Image.open(os.path.join(IMAGE_ROOT, sample['image']))]

        question = sample['question']

        message = build_message_with_images(question, images, max_pixels=round(12_845_056/len(images)))
    ...
    if rank == 0:
        ...
        final_output = []
        matrix = np.zeros((4757, 4), dtype=np.int8)

        for input_example, model_output in zip(data, all_outputs):
            answer = input_example['answer']
            category = input_example['category']
            idx = input_example['id']
            q_type = input_example['q_type']
            ...
            if q_type == 'positive_1':
                matrix[idx, 0] = correct
            elif q_type == 'positive_2':
                matrix[idx, 1] = correct
            else:
                matrix[idx, 2] = correct
