...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'winoground/all_data.json')
    IMAGE_ROOT = os.path.join(args.data_path, 'winoground')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/winoground/', args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        images = [Image.open(os.path.join(IMAGE_ROOT, image)).convert("RGB") for image in sample["image_name"]]
    ...
    if rank == 0:
        ...
        final_output = []
        matrix = np.zeros((400, 7), dtype=np.int8)

        for input_example, model_output in zip(data, all_outputs):
            answer = input_example['GT']
            question_type = input_example['question_type']
            idx = input_example['idx']
        ...
        textscore = matrix[:, 2].mean() * 100
        imagescore = matrix[:, 5].mean() * 100
        groupscore = matrix[:, 6].mean() * 100

        print(f"\ntextscore: {textscore:.2f}%")
        print(f"\nimagescore: {imagescore:.2f}%")
        print(f"\ngroupscore: {groupscore:.2f}%")
        print(f"({textscore:.2f}, {imagescore:.2f}, {groupscore:.2f})")
