...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'HR-Bench')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/hrbench/', args.save_name)
    ...
    import datasets
    data = datasets.load_dataset(DATA_PATH)['hrbench_8k']
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        images = [Image.open(io.BytesIO(base64.b64decode(sample['image']))).convert("RGB")]
        option_letters = ['A', 'B', 'C', 'D']
        option_str = ""
        for option_letter in option_letters:
            if option_letter in sample and len(sample[option_letter]):
                option_str += f"\n{option_letter}. {sample[option_letter]}"
        question = '<image>\n'*len(images) + sample['question'] + option_str + '\nAnswer with the option letter directly.'
    ...
    if rank == 0:
        ...
        for input_example, model_output in zip(data, all_outputs):
            ...
            task_type = input_example['category']
