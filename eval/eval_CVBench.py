...
if __name__ == "__main__":
    ...
    DATA_PATH = os.path.join(args.data_path, 'CVBench/all_data.json')
    VIDEO_ROOT = os.path.join(args.data_path, 'CVBench/videos')
    MODEL_PATH = args.model_path
    OUTPUT_PATH = os.path.join('./logs/CVBench/', f"frame{max_frames}_pixel{pix}_"+args.save_name)
    ...
    for sample in tqdm(rank_data, disable=rank != 0):
        videos = [os.path.join(VIDEO_ROOT, video) for video in sample['videos']]
        question = "<video>\n"*len(videos) + sample['question']

        message = build_message_with_videos(question, videos, fps=fps, max_frames=max_frames//4, max_pixels=max_pixels, min_pixels=min_pixels)
